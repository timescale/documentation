...
<Icon icon="tag" iconType="duotone" /> Since [0.5.0][tsdb-0.5.0]
...
|-|-|-|
| `histogram` | BIGINT[ ] | An array of counts, with `nbuckets` + 2 elements. The first element is the count of values less than `min`, the last element is the count of values greater than or equal to `max`, and the middle elements are counts for each bucket in the range. |
