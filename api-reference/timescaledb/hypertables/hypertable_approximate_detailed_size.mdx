...
import ReturnsNullIfNotHypertable from '/snippets/api-reference/timescaledb/_returns-null-if-not-hypertable.mdx';
import { HYPERTABLE, HYPERTABLE_CAP, CHUNK, CHUNK_CAP, CAGG, PG } from '/snippets/vars.mdx';

<Icon icon="circle-play" iconType="duotone" /> Since 2.13.0
...
```sql
SELECT hypertable_approximate_detailed_size(
    relation = '<hypertable_name>'
);
```
