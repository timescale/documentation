...
<Icon icon="circle-play" iconType="duotone" /> Since 1.5.0

Old API since [{TIMESCALE_DB} v2.18.0](https://github.com/timescale/timescaledb/releases/tag/2.18.0). Superseded by [ALTER TABLE (Hypercore)][alter-table-hypercore].
However, compression APIs are still supported, you do not need to migrate to the hypercore APIs.

Use 'ALTER TABLE' to turn on compression and set compression options.
