...
description: Understand the different compression methods, which includes delta encoding, delta-of-delta, simple-8b,
  run-length encoding, XOR-based, and dictionary compression
...
1. **TimescaleDB dictionary compression**: {TIMESCALE_DB} attempts to apply
...
2. **PostgreSQL TOAST compression**: if dictionary compression does not perform, {TIMESCALE_DB} skips compression and allows {PG} to handle the data using
