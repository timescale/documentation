...
1. Create a new policy on `conditions_summary_hourly` that keeps the {CAGG} up to date, and runs every
   hour:
...
1. Create a new policy on `conditions_summary_hourly` that keeps data removed from the {HYPERTABLE} in
...
1. Create a new policy on `conditions_summary_daily` to refresh the {CAGG} with recently inserted data
   which runs hourly:
...
2. Create a concurrent policy on `conditions_summary_daily` to refresh the {CAGG}
...
[data-retention]: /manage-data/features/data-retention
