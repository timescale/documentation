...
description: Manage Tiger Cloud databases with pgAdmin's graphical interface to query data, design schemas, administer users, and monitor performance.
keywords: [pgAdmin, PostgreSQL, database administration, open-source, query tool, database management, GUI, SQL editor]
---


import IntegrationPrereqs from '/snippets/prerequisites/_integration-prereqs.mdx';
import { PG, CLOUD_LONG, SERVICE_SHORT } from '/snippets/vars.mdx';

[pgAdmin][pgadmin] is available for Chrome, Firefox, Edge, and Safari browsers, or can be installed on Microsoft
 Windows, Apple macOS, or various Linux flavors.
