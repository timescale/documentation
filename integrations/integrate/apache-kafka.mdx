...
description: Stream events from Kafka topics into hypertables using Kafka Connect and PostgreSQL sink connector for real-time analytics on streaming data.
keywords: [Apache Kafka, Kafka Connect, event streaming, data pipelines, real-time streaming, distributed systems, message broker, producers, consumers]
---
...
- Install [Java8 or higher][java-installers] to run Apache Kafka
...
2. **Query your {SERVICE_LONG} for all rows in the `accounts` table**
