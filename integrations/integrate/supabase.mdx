...
description: Run real-time analytical queries from Supabase using foreign data wrappers to access hypertables, continuous aggregates, and time-series data.
...
This page shows how to run real-time analytical queries against a {SERVICE_LONG} through [Supabase][supabase] using a
foreign data wrapper (fdw) to bring aggregated data from your {SERVICE_LONG}.
...
1. **Optimize time-series data in hypertables**
   ...
   time.
    ...
    [Connect to your {SERVICE_LONG}][connect] and create a table that will point to a Supabase database:
...
1. **Create optimized analytical queries**
   ...
   1. Set up a delay stats comparing `origin_time` to `time`.
   ...
   1. Set up a view to receive the data from Supabase.
