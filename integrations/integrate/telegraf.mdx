...
description: Ingest data into a Tiger Cloud service using using the Telegraf plugin
keywords: [Telegraf, data ingestion, metrics collection, InfluxData, plugins, IoT, systems monitoring, agent, time-series data]
---

import { PG, HYPERTABLE, TIMESCALE_DB } from '/snippets/vars.mdx';
import ImportPrerequisites from '/snippets/prerequisites/_migrate-import-prerequisites.mdx';
import SetupConnectionString from '/snippets/procedures/_migrate-import-setup-connection-strings.mdx';
...
1. **Configure the {PG} output plugin**
   ...
   1. Use {HYPERTABLE}s when Telegraf creates a new table:
      ...
      The `by_range` dimension builder was added to {TIMESCALE_DB} 2.13.
...
see the [PostgreSQL output plugin][output-plugin].
...
[create-service]: /getting-started/
[connect-timescaledb]: /integrations/find-connection-details
