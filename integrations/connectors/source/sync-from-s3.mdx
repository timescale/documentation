...
import { CAGG, CLOUD_LONG, COLUMNSTORE, COMPANY, CONSOLE, CONTACT_SALES, HYPERTABLE, PROJECT_SHORT, S3_CONNECTOR, S3_CONNECTOR_CAP, SERVICE_LONG, SERVICE_SHORT, SQL_EDITOR } from '/snippets/vars.mdx';
...
- The {S3_CONNECTOR} offers an option to enable a [{HYPERTABLE}][hypertables-section] during the file-to-table schema mapping setup. You can enable [{COLUMNSTORE}][compression] and [{CAGG}s][caggs] through the {SQL_EDITOR} once the connector has started running.
...
The {S3_CONNECTOR_CAP} continuously imports data from an Amazon S3 bucket into your database. It monitors your S3 bucket for new files matching a specified pattern and automatically imports them into your designated database table.
...
If you have any questions or feedback about the {S3_CONNECTOR}, join us in the [{COMPANY} community][join-livesync-on-slack].
  ...
  The following credentials are supported:
    - [IAM Role][credentials-iam].
      ...
      - Configure the trust policy. Set the:
        ...
        - `ExternalID`: set to the [{CLOUD_LONG} {PROJECT_SHORT} and {SERVICE_LONG} ID][get-project-id] of the {SERVICE_SHORT} you are syncing to in the format `<projectId>/<serviceId>`.
...
- **CSV**:
   - Maximum file size: 1 GB
      ...
      To increase this limit, contact {CONTACT_SALES}
...
1. **Optimize the data to synchronize in hypertables**
   ...
   {CONSOLE} checks the file schema and, if possible, suggests the column to use as the time dimension in a [{HYPERTABLE}][hypertables-section].
...
1. **Monitor synchronization**
    ...
    The {S3_CONNECTOR_CAP} provides comprehensive observability that gives you maximum visibility on connector performance.
...
And that is it, you are using the {S3_CONNECTOR} to synchronize all the data, or specific files, from an S3 bucket to your {SERVICE_LONG} in real time.
