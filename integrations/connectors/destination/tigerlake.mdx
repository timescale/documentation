...
description: Sync hypertables and relational tables to Amazon S3 Tables using Iceberg format for unified analytics across operational and data lake architectures.
keywords: [Tiger Lake, data lake, destination connector, Iceberg, S3 Tables, Amazon S3, synchronization, hypertables, real-time, ETL]
---
...
import { LAKE_LONG, LAKE_SHORT, CLOUD_LONG, SERVICE_LONG, SERVICE_SHORT, HYPERTABLE, HYPERTABLE_CAP, CONSOLE, PG } from '/snippets/vars.mdx';
...
running in {SERVICE_LONG}s to Iceberg tables running in [Amazon S3 Tables][s3-tables] in your AWS account.
...
1. **Set the AWS region to host your table bucket**
   1. In [AWS CloudFormation][aws-cloudformation], select the current AWS region at the top-right of the page.
...
3. **Connect your {SERVICE_SHORT} to the data lake**
   ...
   1. In [{CONSOLE}][console-services], select the {SERVICE_SHORT} you want to integrate with AWS S3 Tables, then click
      `Connectors`.
...
1. **Create a S3 Bucket**
   ...
   1. Set the AWS region to host your table bucket
      1. In [Amazon S3 console][aws-s3-console], select the current AWS region at the top-right of the page.
...
2. **Create an ARN role**
   1. In [IAM Dashboard][aws-iam-dashboard], click `Roles` then click `Create role`
...
| `truncate[W]` | Value truncated to width W, see [options][iceberg-truncate]       |

These partitions define the behavior using the [Iceberg partition specification][iceberg-partitions].
...
* [Amazon S3 Tables Iceberg REST][s3-iceberg-rest] catalog only is supported.
* In order to collect deletes made to data in the columstore, certain columnstore optimizations are disabled for {HYPERTABLE}s, this includes [Direct Compress][direct-compress].
...
[aws-iam-dashboard]: https://console.aws.amazon.com/iamv2/home
[iceberg-truncate]: https://iceberg.apache.org/spec/#truncate-transform-details
[iceberg-partitions]: https://iceberg.apache.org/spec/#partition-transforms
[s3-iceberg-rest]: https://docs.aws.amazon.com/AmazonS3/latest/userguide/s3-tables-integrating-open-source.html
[data-tiering]: /use-timescale/data-tiering/
[direct-compress]: /use-timescale/hypertables/hypertable-crud#speed-up-data-ingestion