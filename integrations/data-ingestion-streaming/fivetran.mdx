...
keywords: [connect, integrate, fivetran, etl, data pipeline]
---

import IntegrationPrereqs from '/snippets/prerequisites/_integration-prereqs.mdx';
import { CLOUD_LONG, CONSOLE, PG, SERVICE_LONG, SERVICE_SHORT } from '/snippets/vars.mdx';
...
1. Search for the `{PG}` connector and click `Select`. Add the destination name and click `Add`.
1. In the `{PG}` setup, add your [{SERVICE_LONG} connection details][connection-info], then click `Save & Test`.
