...
Real-time aggregates combine materialized data with real-time calculations on recent data. However, if you update, insert, or delete data in a previously materialized {TIME_BUCKET}, those changes are not visible in your {CAGG} until you refresh that {TIME_BUCKET} again.

To see updates to previously materialized regions, you must manually refresh the {CAGG} for the affected time range using `refresh_continuous_aggregate()`.
