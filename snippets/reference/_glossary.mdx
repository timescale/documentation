...
import { CLOUD_LONG, COMPANY, HYPERTABLE, CHUNK, CHUNK_CAP, CHUNK_SKIPPING_CAP, COLUMNSTORE_CAP, CAGG_CAP, HA_REPLICA_CAP, HYPERCORE_CAP, HYPERTABLE_CAP, IO_BOOST, LIVESYNC_CAP, PRICING_PLAN_CAP, PERFORMANCE, SCALE, ENTERPRISE, READ_REPLICA_CAP, ROWSTORE_CAP, SERVICE_LONG, SERVICE_SHORT, TIME_BUCKET_CAP, TIMESCALE_DB, PG, VPC } from '/snippets/vars.mdx';
...
**{CHUNK_CAP}**: a horizontal partition of a {HYPERTABLE} that contains data for a specific time interval and space partition.
...
**{COLUMNSTORE_CAP}**: {TIMESCALE_DB}'s columnar storage engine optimized for analytical workloads and compression.
...
**Free {SERVICE_SHORT}**: a free instance of {CLOUD_LONG} with limited resources. You can create up to two free {SERVICE_SHORT}s under any pricing plan. When a free {SERVICE_SHORT} reaches the resource limit, it converts to the read-only state. You can convert a free {SERVICE_SHORT} to a standard one under paid pricing plans.
...
**{HYPERCORE_CAP}**: {TIMESCALE_DB}'s hybrid storage engine that seamlessly combines row and column storage for optimal performance.

**Hyperfunction**: an SQL function in {TIMESCALE_DB} designed for time-series analysis, statistics, and specialized computations.
...
**{HYPERTABLE_CAP}**: {TIMESCALE_DB}'s core abstraction that automatically partitions time-series data for scalability.
...
**pgai on {CLOUD_LONG}**: a cloud solution for building search, RAG, and AI agents with {PG}. Enables calling AI embedding and generation models directly from the database using SQL.
...
**{READ_REPLICA_CAP}**: a copy of the primary database that serves read-only queries, improving read scalability and geographic distribution.
...
**{ROWSTORE_CAP}**: traditional row-oriented data storage where data is stored row by row, optimized for transactional workloads.
...
**{SERVICE_LONG}**: an optimized {PG} instance extended with database engine innovations such as {TIMESCALE_DB}, in a cloud infrastructure that delivers speed without sacrifice.
...
**Standard {SERVICE_SHORT}**: a regular {SERVICE_LONG} that includes the resources and features according to the pricing plan. You can create standard {SERVICE_SHORT}s under any of the paid plans.
...
**{CLOUD_LONG}**: {COMPANY}'s managed cloud platform that provides {TIMESCALE_DB} as a fully managed solution with additional features.
