...
import { CAGG, CHUNK, CLOUD_LONG, COMPANY, HYPERTABLE, HYPERTABLE_CAP, PG, SELF_LONG_CAP, SERVICE_LONG, TIMESCALE_DB } from '/snippets/vars.mdx';
...
Every {SERVICE_LONG} is a 100% {PG} database hosted in {CLOUD_LONG} with {COMPANY} extensions such as {TIMESCALE_DB}. You connect to your {SERVICE_LONG} from a standard Rails app configured for {PG}.
...
1. **Connect your app to your {SERVICE_LONG}**
   ...
   1. In `<my_app_home>/config/database.yml` update the configuration to read securely connect to your {SERVICE_LONG} by adding `url: <%= ENV['DATABASE_URL'] %>` to the default configuration:
   ...
   1. Set the environment variable for `DATABASE_URL` to the value of `Service URL` from your [connection details][connection-info]
...
{HYPERTABLE_CAP}s are {PG} tables designed to simplify and accelerate data analysis. Anything you can do with regular {PG} tables, you can do with {HYPERTABLE}s - but much faster and more conveniently.
...
1. **Add hypertable options**
   ...
   Replace the contents of `<my_app_home>/db/migrate/<migration-datetime>_create_page_loads.rb` with the following:
    ...
    The `id` column is not included in the table. This is because {TIMESCALE_DB} requires that any `UNIQUE` or `PRIMARY KEY` indexes on the table include all partitioning columns. In this case, this is the time column.

    A new Rails model includes a `PRIMARY KEY` index for id by default: either remove the column or make sure that the index includes time as part of a "composite key."

   For more information, check the Ruby docs around [composite primary keys][rails-compostite-primary-keys].
...
The {TIMESCALE_DB} gem provides efficient ways to insert data into {HYPERTABLE}s. This section
...
The {TIMESCALE_DB} gem provides utility methods to access {HYPERTABLE} and {CHUNK} information. Every model that uses the `acts_as_hypertable` method has access to these methods.
...
- Compress/Decompress {CHUNK}s:

    ```ruby
    PageLoad.chunks.uncompressed.first.compress!
    PageLoad.chunks.compressed.first.decompress!
    PageLoad.hypertable.compression_stats
    ```
...
- Get basic {HYPERTABLE} information:
    ...
    hypertable.schema_name
    ```

- Get detailed size information:

    ```ruby
    hypertable.detailed_size
    hypertable.compression_stats
    hypertable.chunks_detailed_size
    hypertable.approximate_row_count
    hypertable.dimensions.map(&:column_name)
    hypertable.continuous_aggregates.map(&:view_name)
    ```

#### Continuous aggregates

The `continuous_aggregates` method generates a class for each {CAGG}.

- Get all the {CAGG} classes:

   ```ruby
   PageLoad.descendants
   ```
...
- Create or drop a {CAGG}:

  Create or drop all the {CAGG}s in the proper order to build them hierarchically. See more about how it works in this [blog post][ruby-blog-post].
