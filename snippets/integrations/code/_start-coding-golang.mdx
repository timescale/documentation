...
3.  You can check that you're connected to your database with this
    hello world program:
...
4.  This example `main.go` generates sample data and inserts it into
...
4.  This example program runs a query, and accesses the results of
    that query:
...
*   Want fast inserts on CSV data? Check out
    [{TIMESCALE_DB} parallel copy](https://github.com/timescale/timescaledb-parallel-copy), a tool for fast inserts,
    written in Go.