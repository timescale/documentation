import { CLOUD_LONG, TIMESCALE_DB, PG } from '/snippets/vars.mdx';
...
- Install the [PGX driver for Go][pgx-driver].
...
1.  [Locate your {TIMESCALE_DB} credentials][connection-info] and compose a [libpq connection string][libpq-connstring],
    for PGX in the following format:

    ```go
    connStr := "postgres://username:password@host:port/dbname"
    ```

    If you're using a hosted version of {TIMESCALE_DB}, or if you need an SSL
...
1.  Check that you're connected to your database with this hello world program:

    <Tabs>
    <Tab title="Single connection">
    ```go
    package main
...
1.  Formulate the `SELECT` statement to convert the table into a hypertable. You
    ...
    [`create_hypertable` docs](/api/hypertable/create_hypertable):
...
*   Get up and running with {TIMESCALE_DB} with the [Getting Started](/getting-started/latest/)
    tutorial.
