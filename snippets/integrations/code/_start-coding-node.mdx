...
*   Install the Node.js package manager [npm][install-npm].

## Connect to TimescaleDB

In this section, you create a connection to {TIMESCALE_DB} with a common Node.js
ORM (object relational mapper) called [Sequelize][sequelize].
...
6.  Locate your {TIMESCALE_DB} credentials and use them to compose a connection
...
7.  Compose your connection string variable, using this format:

    ```javascript
    'postgres://<user>:<password>@<host>:<port>/<dbname>'
    ```

8.  Open the `index.js` file you created. Require Sequelize in the application,
...
9.  You can test the connection by adding these lines to `index.js` after the
    `app.get` statement:
