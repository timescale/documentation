import { PG, CLOUD_LONG, COMPANY, TIMESCALE_DB, SELF_LONG_CAP } from '/snippets/vars.mdx';
...
In this section, you use the helpers in the {TIMESCALE_DB} gem to create and manage a [hypertable](/use-timescale/hypertables).
...
1. **Add hypertable options**
   ...
   For more information, check the Ruby docs around [composite primary keys][rails-composite-keys].
...
### Alternative model configuration approaches

The {TIMESCALE_DB} gem provides three ways to configure your models as hypertables:

1. **Per-model configuration** (shown above): Use `extend Timescaledb::ActsAsHypertable` in each model

2. **Global configuration via initializer**: Create `config/initializers/timescaledb.rb`:

    ```ruby
    Timescaledb.configure do |config|
      config.default_hypertable_options = {
        time_column: 'created_at',
        chunk_time_interval: '1 day'
      }
    end
...
3. **Abstract model inheritance**: Create a base class that other models inherit from:

    ```ruby
    class HypertableRecord < ApplicationRecord
      self.abstract_class = true
      extend Timescaledb::ActsAsHypertable
    end

    class PageLoad < HypertableRecord
      acts_as_hypertable time_column: "created_at"
    end
    ```
...
In addition to time-based partitioning, you can add secondary dimensions using the `partition_column` option:

```ruby
hypertable_options = {
  time_column: 'created_at',
  partition_column: 'device_id',  # Add a secondary dimension
  number_partitions: 4,            # Number of partitions for the secondary dimension
  chunk_time_interval: '1 day'
}
```

Multi-dimensional partitioning is useful when you have a high-cardinality dimension (like `device_id`, `customer_id`, or `region`) that you frequently query alongside time.
...
- Get basic hypertable information:
    ...
    hypertable.hypertable_schema # The schema where the hypertable is located
    ```
...
* Follow the [LTTB][lttb-tutorial], [Open AI long-term storage][openai-tutorial], and [candlesticks][candlestick-tutorial] tutorials.

[install-rails]: https://guides.rubyonrails.org/install_ruby_on_rails.html#installing-rails
[rails-composite-keys]: https://guides.rubyonrails.org/active_record_composite_primary_keys.html
...
[lttb-tutorial]: https://timescale.github.io/timescaledb-ruby/toolkit_lttb_tutorial/
[openai-tutorial]: https://timescale.github.io/timescaledb-ruby/chat_gpt_tutorial/
[candlestick-tutorial]: https://timescale.github.io/timescaledb-ruby/toolkit_candlestick/
