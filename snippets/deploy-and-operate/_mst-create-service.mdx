...
import { MST_LONG, MST_SHORT, MST_SERVICE_SHORT, TIMESCALE_DB, MST_CONSOLE_SHORT, MST_CONSOLE_LONG, PG } from '/snippets/vars.mdx';
import MSTIntro from "/snippets/intros/_mst-intro.mdx";
import CloudMSTComparison from "/snippets/intros/_cloud-mst-comparison.mdx";
...
1.  In the `Services` tab, find the {MST_SERVICE_SHORT} you want to connect to, and check it is marked as `Running`.
1.  Click the name of the {MST_SERVICE_SHORT} you want to connect to see the connection  information. Take a note of the `host`, `port`, and `password`.
1.  On your local system, at the command prompt, connect to the {MST_SERVICE_SHORT}, using your own {MST_SERVICE_SHORT} details:
...
[mst-docs]: /deploy-and-operate/mst
[tutorials]: /tutorials
[contact]: https://www.timescale.com/support
