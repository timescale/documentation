import { COMPANY, PRICING_PLAN } from '/snippets/vars.mdx';

{COMPANY} runs a global support organization with Customer Satisfaction (CSAT) scores above 99%.
Support covers all timezones and is fully staffed at weekend hours.

All paid {PRICING_PLAN}s have free Developer Support through email with a target response time of 1 business
day; we are often faster. If you need 24x7 responsiveness, talk to us about
[Production Support][production-support]. With Production Support, you can request help at any time at our [Support portal][support-portal].

[production-support]: https://www.tigerdata.com/support
[support-portal]: https://portal.support.timescale.com/login